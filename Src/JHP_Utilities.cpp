...
GSErrCode ReleaseAllObservedElements(void)
{
	...
	Int32 nElems;
	err = ACAPI_Element_GetObservedElements(&elemHeads, &nElems);

	if (err != NoError || nElems == 0) { return err; }

	for (int i = 0; i < nElems; ++i)
	{
		ACAPI_Element_DetachObserver((elemHeads)[i]);
	}
...
short JHP_GetLayerAttributeIndex(const char* layerName) //Gets the attribute index of a layer
{
...
GSErrCode JHP_AddToSelection(API_Neig* theNeig)
{
	...
	(*selNeigs)[0] = *theNeig;
	err = ACAPI_Element_Select(selNeigs, 1, true);
...
GSErrCode JHP_GetSelection(API_Neig ***selNeigs, bool newSelectionIfInvalid)
{
	...
	{
		*selNeigs = ClickElements_Neig("Select Some items", API_ZombieElemID, nullptr);
	}
...
GSErrCode JHP_GetSelection(API_Neig* theNeig, bool newSelectionIfInvalid)
{
	...
	{
		UInt32 nSel = BMGetHandleSize((GSHandle)selNeigs) / sizeof(API_Neig); //gets the number of objects selected
		if (nSel == 1)
		{
			*theNeig = (*selNeigs)[0];
		}
		...
		{
			err = ACAPI_Element_Select(nullptr, 0, false);//empty the selection
...
GSErrCode JHP_SelectElements(API_ElemTypeID id, short layer)
{
	GSErrCode				err;

	// ----- Deselect all selected elements -----
	err = ACAPI_Element_Select(nullptr, 0, false);
	if (err != NoError)
